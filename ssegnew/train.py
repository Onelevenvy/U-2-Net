...
# USE_MULTI_GPU = True  # True: 使用所有可见GPU多卡并行训练; False: 仅使用单卡
...
def train():
    ...
    logger.info(
        f"输出类别数: {NUM_CLASSES} ({'二值分割' if NUM_CLASSES == 1 else '多类别分割'})"
    )
    ...
    if USE_MULTI_GPU and torch.cuda.device_count() > 1:
        ...
        logger.info(
            f"有效 Batch Size: {effective_batch_size} (单卡 {BATCH_SIZE} x {torch.cuda.device_count()} GPUs)"
        )
    else:
    ...
    if TENSORBOARD_AVAILABLE:
        ...
        logger.info(
            f"启动 TensorBoard 命令: tensorboard --logdir={TENSORBOARD_LOG_DIR}"
        )
    ...
    gpu_count = torch.cuda.device_count() if USE_MULTI_GPU else 1
    config = {
    ...
    for epoch in range(EPOCH_NUM):
        ...
        if (epoch + 1) % SAVE_EVERY_N_EPOCHS == 0:
            save_path = os.path.join(
                MODEL_SAVE_DIR, f"{MODEL_NAME}_epoch_{epoch+1}.pth"
            )
